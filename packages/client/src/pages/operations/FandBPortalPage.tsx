...
import { UtensilsCrossed, Wine, Building2, RefreshCw, ArrowRight, ClipboardCheck } from 'lucide-react';
...
type OutletKind = 'restaurant' | 'bar';
...
export const FandBPortalPage: React.FC = () => {
  ...
  const [revenue, setRevenue] = useState({ restaurant: 0, bar: 0, revenueToday: 0 });
  const [orders, setOrders] = useState<any[]>([]);

  const load = async (initial = false) => {
    try {
      initial ? setLoading(true) : setRefreshing(true);
      const [dash, rOrders, bOrders] = await Promise.allSettled([
  ...
  const combinedRevenue = revenue.restaurant + revenue.bar;

  return (
    <ShellPage
      eyebrow="F&B · RESTAURANT & BAR"
      title={greeting}
      subtitle="Restaurant and bar services from a single point of sale — orders, charges and receipts together."
      ...
      {loading ? <LoadingState message="Loading the F&B workspace…" /> : (
        <>
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <StatTile label="Property revenue" value={formatCurrency(revenue.revenueToday)} note="Recorded today" icon={Building2} />
          </div>

          <Section title="Your outlets" subtitle="Open a live workstation to take orders">
            <div className="grid gap-4 p-4 sm:grid-cols-2">
              {OUTLETS.map(({ kind, label, to, icon: Icon, tone }) => (
                <button key={kind} onClick={() => navigate(to)} className="group rounded-2xl border border-[#e8ebe8] bg-[#fbfcfa] p-5 text-left transition hover:border-[#cfd8d3] hover:shadow-sm">
                  ...
                  <div className="mt-1 text-[10px] text-[#8a9598]">Take orders and post receipts</div>
