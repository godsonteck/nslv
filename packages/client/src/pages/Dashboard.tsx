...
const PropertyDashboard: React.FC = () => {
  ...
  return (
    <ShellPage eyebrow="NSVILLA · PROPERTY CONTROL" title={greeting} subtitle="A live command centre for reservations, rooms, guests, revenue and department activity." actions={<><Button variant="outline" size="sm" onClick={() => void load()} loading={refreshing}><RefreshCw size={14} /> Refresh</Button>{canReserve && <Button size="sm" onClick={() => navigate('/reservations')}><CalendarCheck size={14} /> New reservation</Button>}</>}>
      ...
      <section className="relative overflow-hidden rounded-[24px] bg-[#0E0B12] text-white shadow-[0_18px_50px_rgba(14,11,18,.38)]">
        <img src={villaAssets.villaExterior} className="absolute inset-0 h-full w-full object-cover opacity-25" alt="" />
        <div className="absolute inset-0 bg-gradient-to-r from-[#0E0B12] via-[#0E0B12]/95 to-[#0E0B12]/45"/>
        <div className="relative grid min-h-[260px] lg:grid-cols-[1.2fr_.8fr]">
          <div className="relative p-7 sm:p-9"><div className="text-[10px] font-extrabold uppercase tracking-[.2em] text-[#f1a83f]">Today's operations</div><h2 className="mt-3 max-w-xl text-3xl font-extrabold tracking-[-.04em] sm:text-4xl">Every guest journey, connected.</h2><p className="mt-3 max-w-lg text-sm leading-6 text-white/70">Move from arrival to checkout without losing the room, folio or department context.</p><div className="mt-6 flex flex-wrap gap-2">{canFrontDesk && <Button size="sm" className="!bg-[#f1a83f] !text-[#201608]" onClick={() => navigate('/frontdesk')}><LogIn size={14} /> Open front desk</Button>}{canReports && <Button size="sm" variant="outline" className="!border-white/20 !bg-white/10 !text-white" onClick={() => navigate('/reports')}><ArrowRight size={14} /> View reports</Button>}</div></div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.35fr_.65fr]">
        <Section title="Guests in house" subtitle="Active stays requiring attention" action={canFrontDesk ? <button onClick={() => navigate('/frontdesk')} className="text-[11px] font-extrabold text-[#a8761e]">Open front desk →</button> : undefined}>
          {stays.length === 0 ? <div className="p-6 text-center text-xs text-[#899397]">No guests are currently checked in.</div> : <div className="divide-y divide-[#edf0ed]">{stays.slice(0, 7).map(s => <div key={s.id} className="flex items-center gap-4 px-5 py-4"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#e9f3f5] text-xs font-extrabold text-[#16a4d4]">{(s.guest?.firstName?.[0] || 'G') + (s.guest?.lastName?.[0] || '')}</div><div className="min-w-0 flex-1"><div className="truncate text-xs font-extrabold text-[#26363e]">{[s.guest?.firstName, s.guest?.lastName].filter(Boolean).join(' ') || 'Guest'}</div><div className="mt-1 text-[10px] text-[#8a9598]">Room {s.room?.number || '—'} · departure {s.checkOutDate ? new Date(s.checkOutDate).toLocaleDateString() : '—'}</div></div>{statusBadge(s.status || 'CHECKED_IN')}</div>)}</div>}
        </Section>
    ...
    <ShellPage eyebrow={`NSVILLA · ${deptLabel.toUpperCase()} OPS`} title={greeting} subtitle="Your department's sales, orders and receipts — no room or guest data required." actions={<><Button variant="outline" size="sm" onClick={() => void load()} loading={refreshing}><RefreshCw size={14} /> Refresh</Button><Button size="sm" onClick={() => navigate(toPOS)}><ArrowRight size={14} /> Open {deptLabel} POS</Button></>}>
      ...
      <section className="relative overflow-hidden rounded-[24px] bg-[#0E0B12] text-white shadow-[0_18px_50px_rgba(14,11,18,.38)]">
        ...
        <div className="relative grid min-h-[220px] lg:grid-cols-[1.2fr_.8fr]">
          <div className="relative p-7 sm:p-9"><div className="text-[10px] font-extrabold uppercase tracking-[.2em] text-[#f1a83f]">Today's {deptLabel.toLowerCase()} sales</div><h2 className="mt-3 max-w-xl text-3xl font-extrabold tracking-[-.04em] sm:text-4xl">{formatCurrency(Number(todayRevenue || 0))}</h2><p className="mt-3 max-w-lg text-sm leading-6 text-white/70">Revenue recorded today across {deptLabel.toLowerCase()} orders and transactions.</p><div className="mt-6 flex flex-wrap gap-2"><Button size="sm" className="!bg-[#f1a83f] !text-[#201608]" onClick={() => navigate(toPOS)}><ArrowRight size={14} /> Take an order</Button></div></div>
      ...
      <Section title="Recent transactions" subtitle="Latest orders and receipts" action={<button onClick={() => navigate(toPOS)} className="text-[11px] font-extrabold text-[#a8761e]">New order →</button>}>
        {orders.length === 0 ? (
...
const Dept = ({ icon: Icon, label, value }: { icon: any; label: string; value: any }) => <div className="flex items-center gap-3 rounded-2xl bg-[#fbfcfa] p-4"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#f4ecdf] text-[#a8761e]"><Icon size={16} /></span><span className="flex-1 text-xs font-bold text-[#526168]">{label}</span><strong className="text-sm text-[#20343e]">{formatCurrency(Number(value || 0))}</strong></div>;
