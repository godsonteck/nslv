    ...
    <ShellPage eyebrow="NSVILLA · PROPERTY CONTROL" title="Good morning. Run the property with clarity." subtitle="A live command centre for reservations, rooms, guests, revenue and department activity." actions={<><Button variant="outline" size="sm" onClick={() => void load()} loading={refreshing}><RefreshCw size={14} /> Refresh</Button>{canReserve && <Button size="sm" onClick={() => navigate('/reservations')}><CalendarCheck size={14} /> New reservation</Button>}</>}>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.35fr_.65fr]">
        <Section title="Guests in house" subtitle="Active stays requiring attention" action={canFrontDesk ? <button onClick={() => navigate('/frontdesk')} className="text-[11px] font-extrabold text-[#8d693c]">Open front desk →</button> : undefined}>
          {stays.length === 0 ? <div className="p-6 text-center text-xs text-[#899397]">No guests are currently checked in.</div> : <div className="divide-y divide-[#edf0ed]">{stays.slice(0, 7).map(s => <div key={s.id} className="flex items-center gap-4 px-5 py-4"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#edf4f2] text-xs font-extrabold text-[#174b59]">{(s.guest?.firstName?.[0] || 'G') + (s.guest?.lastName?.[0] || '')}</div><div className="min-w-0 flex-1"><div className="truncate text-xs font-extrabold text-[#26363e]">{s.guest ? `${s.guest.firstName} ${s.guest.lastName}` : 'Guest'}</div><div className="mt-1 text-[10px] text-[#8a9598]">Room {s.room?.number || '—'} · departure {s.checkOutDate ? new Date(s.checkOutDate).toLocaleDateString() : '—'}</div></div>{statusBadge(s.status || 'CHECKED_IN')}</div>)}</div>}
        </Section>
