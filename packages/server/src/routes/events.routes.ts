...
router.post('/:id/calculate-price', requirePermission('events.view'), async (req, res, next) => {
  try {
    const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    const { startAt, endAt, guestCount } = req.body;
    const gCount = guestCount ? Number(guestCount) : 0;
    const result = await EventsService.calculateBookingPrice(id, new Date(startAt), new Date(endAt), gCount);
